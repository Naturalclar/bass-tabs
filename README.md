...
画面で弾きながら追う用途（カーソル追従・追従スクロール・区間ループ・再生中のテンポ変更）は
**対象外**。このサイトは印刷物を出すためのもので、そちらの用途は alphaTab で別途検討する。
エディタの「再生」は追従ではなく校正のための機能 — 入力した音が合っているかを、
印刷する前に耳で確かめるための一回再生で、それ以上には育てない。
